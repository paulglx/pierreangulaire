...
**Shader specialisation.** Blend mode, segmentation visibility, and the empty-block debug toggle are compile-time constants of the raycast shader (WGSL `override`s), not uniforms. Each combination in use is its own render pipeline, created on first use and cached, so the compiler strips every path the view cannot take — a MIP viewport with the overlay hidden marches a loop of step, gate, fetch, max, with no segment-set bookkeeping and no live-register cost for the other modes.
...
- **GPURenderer** (§7, §14): WebGPU orthographic slab raycast in WGSL, one canvas context per viewport. Each image volume is one dense 3D texture in its native texel format (`r16sint` / `r16uint` / `r8uint` / `r32float`) at the volume's own dimensions, allocated at creation and filled brick by brick in place as bricks become resident; a volume larger than `maxTextureDimension3D` on any axis is rejected at creation. The raycast reconstructs trilinear samples from 8 clamped `textureLoad` taps at voxel indices, applies the rescale in the shader, and skips non-resident bricks through the per-brick texel. A `r32sint` **page-table texture** (one texel per brick) carries only the segmentation atlas slot (0 = unpainted). One shader module and bind-group layout per texel type (`f32` / `i32` / `u32`); one render pipeline per (texel type, blend mode, segmentation visibility, debug toggle) combination, specialised through WGSL `override` constants (§7.2) and created on first use.
