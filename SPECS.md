...
| Image voxel storage            | Native format — CPU per-brick arrays, one dense GPU 3D texture      |
...
- **GPURenderer** (§7, §14): WebGPU orthographic slab raycast in WGSL, one canvas context per viewport. Each image volume is one dense 3D texture in its native texel format (`r16sint` / `r16uint` / `r8uint` / `r32float`) at the volume's own dimensions, allocated at creation and filled brick by brick in place as bricks become resident; a volume larger than `maxTextureDimension3D` on any axis is rejected at creation. The raycast reconstructs trilinear samples from 8 `textureLoad` taps at voxel indices (the base and next corners are clamped once, not per tap), applies the rescale once to the interpolated value, and skips non-resident bricks through the per-brick texel. A second dense 3D texture at a quarter of the volume's dimensions carries the 4³ **cell ranges** (§5.1) in the volume's native two-channel format, written per brick alongside the voxels. A `r32sint` **page-table texture** (one texel per brick) carries only the segmentation atlas slot as a packed slot coordinate (10 bits per axis, offset by one so 0 = unpainted). One bind-group layout per texel type (`f32` / `i32` / `u32`); one shader module per (texel type, segmentation visibility) since the overlay-hidden variant has a single color output; one render pipeline per (texel type, blend mode, segmentation visibility, debug view) combination, specialised through WGSL `override` constants (§7.2) and created on first use. When the adapter offers `timestamp-query` each viewport carries a two-entry query set plus resolve and readback buffers, and the measured frame time is published on the viewport.
...
- The image volume is a single dense texture: there is no residency budget or eviction, so a volume must fit in GPU memory in its native format (plus about 3% for the cell-range texture) and within `maxTextureDimension3D` per axis. Because volume textures and the staging for brick uploads can be large, the device is requested with the adapter's maximum `maxBufferSize` and `maxTextureDimension3D`.
