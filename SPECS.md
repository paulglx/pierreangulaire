...
**Segmentation-space skipping.** The same per-brick texel carries a segmentation occupancy flag, dilated by one brick so the overlay's few-voxel neighborhood reads (border test, antialiasing kernel) can never reach a labeled voxel from a brick marked empty. Samples in unmarked bricks skip segmentation sampling entirely — the dominant overlay cost in thick slabs, where most of a ray crosses unlabeled space. Like the value-range gate, this never changes output.
