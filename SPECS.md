...
| Image voxel storage            | Native format, bricked — CPU per-brick arrays, GPU brick pool       |
...
- **GPURenderer** (§7, §14): WebGPU orthographic slab raycast in WGSL, one canvas context per viewport. Each volume's bricks live in a **brick pool** — a 3D atlas texture in the volume's native texel format (`r16sint` / `r16uint` / `r8uint` / `r32float`), shaped as close to a cube as `maxTextureDimension3D` allows and sized for every brick of the volume up front — addressed through an `rg32sint` **page-table texture** (one texel per brick: image pool slot and segmentation atlas slot, 0 = absent). Bricks are uploaded into a slot as they become resident. The raycast resolves each voxel fetch as brick coordinate → page-table lookup → pool coordinate, reconstructs trilinear samples from 8 `textureLoad` taps (each through the page table, so taps straddling a brick border read the neighbor brick), applies the rescale in the shader, and skips `Absent` bricks. One shader module and bind-group layout per texel type (`f32` / `i32` / `u32`); one render pipeline per (texel type, blend mode, segmentation visibility, debug toggle) combination, specialised through WGSL `override` constants (§7.2) and created on first use.
...
- The image brick pool is allocated at full capacity when the volume is created (every brick of a fully loaded volume becomes resident) and slots are never evicted: there is no residency budget, so a volume must fit in GPU memory in its native format. Because pool textures and the staging for brick uploads can be large, the device is requested with the adapter's maximum `maxBufferSize` and `maxTextureDimension3D`.
- Trilinear image reconstruction is a manual 8-tap `textureLoad`, satisfying the Linear default; bricks carry no apron, so a hardware sampler could not filter across brick borders.
